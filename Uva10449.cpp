...
void solve(i32 n){
    ...
    // i32 n,m;see(n,m);
    vector<i32> val(n+1);
    rep(i,1,n+1) see(val[i]);
    i32 m;see(m);
    rep(i,0,m){
        i32 u,v;see(u,v);
        i32 w = exp(val[v]-val[u],3);
        adj[u].push_back({v,w});
    }
    
    vector<i32> count(n+1,0),in_queue(n+1,0),dist(n+1,INF);
    queue<i32> q;q.push(1);
    dist[1]=0;
    while(!q.empty()){
        i32 u = q.front();q.pop();
        in_queue[u]=0;
        count[u]++;
        if(count[u]>n) continue;
        for(auto &[v,w]:adj[u]){
    ...
    while(qr--){
        i32 x;see(x);
        if(dist[x]<3 || dist[x]>=INF || count[x]>=n) putln("?");
        else putln(dist[x]);
    }
