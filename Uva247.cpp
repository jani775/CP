...
// set<i32> adj[MAXN];
// vector<i32> adj[MAXN],tAdj[MAXN];
// vector<i32> bridgeAdj[MAXN];
// char grid[MAXN][MAXN];
...
// void sccKosaraju(i32 node,vector<bool> &visited,i32 pass,stack<int> &st){
    // debug(node,visited,pass,st);
//     visited[node] = true;
//     vector<i32> temp = (pass==1) ? adj[node] : tAdj[node];
//     for(auto &x : temp){
//         if(!visited[x]){
//             sccKosaraju(x,visited,pass,st);
//         }
//     }
//     st.push(node);
    // debug(node,st,visited);
// }

void sccTarjan(string node,set<string> &visited,map<string, i32> &in,map<string, i32> &low,stack<string> &stk){
    // debug(node,visited,stk,in,low,ans);
    in[node] = low[node] = dfsCounter++;
    visited.insert(node);
    stk.push(node);
    // debug(node,visited,stk,in,low,ans);
    for(auto &x : adj[node]){
        if(in.count(x)==0){
            sccTarjan(x,visited,in,low,stk);
        }
...
void solve(i32 n,i32 m,i32 tc){
    ...
    // debug(adj);
    set<string> visited;
    map<string, i32> in,low;
    stack<string> stk;
    for(auto &[u,v]:adj){
        if(in.count(u)==0){
        // debug(u);
            sccTarjan(u,visited,in,low,stk);
        }
    }
    // debug(scc,ans,stk,in,low);
    putn("Calling circles for data set",tc);
    putln(":");
    for(auto &x:ans){
        rep(i,0,x.size()){
            if(i!=0) putn(" ");
