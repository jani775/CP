...
void dfs(i32 node,vector<i32> &visited,i32 u,i32 v,i32 w,bool &check){
    visited[node] = w;
    bool ans = false;
    for(auto x:adj[node]){
        if(visited[x]==-1){
            if(w==u){
                dfs(x,visited,u,v,v,check);
            }else{
                dfs(x,visited,u,v,u,check);
            }
        }else{
            if(visited[x]==w) check = true;
        }
    }
}
...
void solve(){
    ...
    rep(i,1,n+1){
        if(visited[i]==-1){
            bool check = false;
            dfs(i,visited,2*t,2*t+1,2*t,check);
            i32 a = 0,b=0;
            rep(i,1,n+1){
                if(visited[i]==2*t) a++;
                else if(visited[i]==2*t+1) b++;
            }
            i32 c = max(a,b);
            if(!check)ans+=c;
            // debug(c,ans);
