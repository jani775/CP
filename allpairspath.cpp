...
void solve(i32 n,i32 m,i32 q){
    ...
    vector<i32> neg_cycle;
    rep(i,0,n){
        if(grid[i][i]<0) neg_cycle.push_back(i);
    }

    while(q--){
        i32 u,v;see(u,v);
        bool ans = false;
        for(auto &x:neg_cycle){
            if((grid[u][x]<INF)&&(grid[x][v]<INF)){
                ans = true;
                break;
            }
        }
        if(ans) putln("-Infinity");
        else if(grid[u][v]==INF) putln("Impossible");
        else putln(grid[u][v]);
    }
    cout<<"\n";
}
