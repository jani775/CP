...
bool dfs(i32 node,i32 color,vector<i32> &visited){
    // debug(node,color,visited);
    visited[node] = color;
    bool ans = false;
    for(auto x : adj[node]){
        // debug(node,color,x,visited[x]);
        if(visited[x]==-1){
            ans |= dfs(x,color!=1,visited);
        }else{
            if(visited[x]==color) return true;
        }
    }
    return ans;
}
