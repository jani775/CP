...
void solve(i32 n,i32 m){
    ...
    vector<vector<i32>> dist(n,vector<i32>(m,INF));
    vector<vector<pair<i32, i32>>> parent(n,vector<pair<i32, i32>>(m,{0,0}));
    min_heap<tuple<i32,i32,i32>> pq;
    rep(i,0,m){
        dist[0][i]=grid[0][i]-'0';
        parent[0][i]={0,i};
        pq.emplace(dist[0][i],0,i);
    }
    // debug(pq,dist,parent);
    while(!pq.empty()){
        auto [u,v,w] = pq.top();pq.pop();
        // debug(u,v,w,dist[v][w]);
        if(dist[v][w]<u) continue;
        rep(i,0,8){
            i32 _x = diaSteps[0][i],_y= diaSteps[1][i];
            // debug(isOk(v+_x,w+_y,grid),v+_x,w+_y,_x,_y);
            if(isOk(v+_x,w+_y,grid)){
                // debug(v+_x,w+_y,v,w);
                if(dist[v][w]+(grid[v+_x][w+_y]-'0')>=dist[v+_x][w+_y]) continue;
                dist[v+_x][w+_y] = dist[v][w]+(grid[v+_x][w+_y]-'0');
                parent[v+_x][w+_y] = {v,w};
                pq.emplace(dist[v+_x][w+_y],v+_x,w+_y);
            }
        }
        // debug(pq);
    }
    // debug(dist,parent);
    i32 minVal = min_element(dist[n-1].begin(),dist[n-1].end())-dist[n-1].begin();
    // debug(minVal);

    i32 i=n-1,j=minVal;
    grid[i][j]='0';
    // debug(i,j);
    while((parent[i][j].ff!=i)||(parent[i][j].ss!=j)){
        // debug(parent[i][j]);
        grid[parent[i][j].ff][parent[i][j].ss] = '0';
        auto [u,v] = parent[i][j];
        i=u;j=v;
        // debug(i,j,parent[i][j]);
    }
    // debug(parent[i][j]);
    rep(i,0,n){
        rep(j,0,m){
            if(grid[i][j]!='0')putn(grid[i][j]);
            else putn(" ");
        }
        cout<<"\n";
    }
