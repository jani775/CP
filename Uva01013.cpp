...
vector<pair<i32, double>> adj[MAXN];
...
void solve(i32 n){
    ...
    rep(i,0,n){
        i32 u,v,w;see(u,v,w);
        cordinates.push_back(make_tuple(u,v,w));
        // edges.push_back(make_tuple(u,v,w));
    }
    rep(i,0,n){
        auto &[x1,y1,w] = cordinates[i];
        rep(j,0,n){
            if(i!=j){
                auto &[x2,y2,w2] = cordinates[j];
                double dst = dist(x1,y1,x2,y2);
                edges.push_back(make_tuple(i,j,dst));
            }
        }
    }
    // debug(edges);
    ...
    // debug(adj);
    vector<double> mxDist(n,0);
    vector<bool> visited(n,false);
    dfs(0,visited,0,mxDist);
    // debug(visited,mxDist);
    double totalTime = 0.0;
    i32 totalPopulation = 0;
    rep(i,0,n){
        auto &[u,v,w] = cordinates[i];
        totalPopulation+=w;
        totalTime += (mxDist[i]*(double)w);
        // debug(totalTime);
    }
    double ans = totalTime/totalPopulation;
    // debug(ans,totalTime,totalPopulation);
    decput(2,ans);
}
