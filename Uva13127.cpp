...
void solve(i32 n,i32 m,i32 b,i32 p){
    ...
    vector<i32> banks(b),police(p);
    rep(i,0,b) see(banks[i]);
    rep(i,0,p) see(police[i]);
    min_heap<pair<i32, i32>> pq;
    vector<i32> dist(n,INF);
    rep(i,0,p){
        dist[police[i]]=0;
        pq.emplace(dist[police[i]],police[i]);
    }
    while(!pq.empty()){
    ...
    i32 x = 0;
    rep(i,0,b){
        x = max(x,dist[banks[i]]);
    }
    rep(i,0,b) {
        if(dist[banks[i]]==x) maxDist.push_back(banks[i]);
    }
    sort(all(maxDist));
    if(x==INF) putln(maxDist.size(),'*');
    else putln(maxDist.size(),x);
    // for(auto y:maxDist) put(y);
    // cout<<"\n";
    rep(i,0,maxDist.size()){
        if(i!=0) cout<<" ";
        cout<<maxDist[i];
    }
    cout<<"\n";
