...
//     return ans;
// }
i32 dfs(i32 i,i32 j,i32 n,i32 m,vector<string> &grid){
    grid[i][j]='w';
    i32 ans =1;
    // debug(i,j);
    rep(k,0,4){
        ...
        if(grid[u][v]=='l') ans+=dfs(u,v,n,m,grid);
    }
    if(j==0){
        if(grid[i][m-1]=='l') ans+=dfs(i,m-1,n,m,grid);
    }
    if(j==m-1){
        if(grid[i][0]=='l') ans+=dfs(i,0,n,m,grid);
    }
    return ans;
}
...
void solve(i32 n,i32 m){
    ...
    rep(i,0,n){
        rep(j,0,m){
            if(grid[i][j]=='l') ans = max(ans,dfs(i,j,n,m,grid));
        }
    }
    putln(ans);
}
