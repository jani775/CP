...
vector<i32> dfs(i32 u,i32 v,i32 n,i32 m,i32 step,vector<vector<i32>> &visited){
    visited[u][v] = step;
    i32 a=0,b=0;
    if(grid[u][v]=='W') b=-1;
    else if(grid[u][v] == 'E') b=1;
    else if(grid[u][v] == 'S') a =1;
    else a = -1;
    // debug(u,v,a,b,n,m,step);
    u+=a;v+=b;
    // debug(u,v,n,m,step);
    vector<i32> ans;
    if((u<0)||(v<0)||(u>=n)||(v>=m)) return {1,step};
    else if(visited[u][v]!=-1) return {0,visited[u][v]-1,step-visited[u][v]+1};
    else ans = dfs(u,v,n,m,step+1,visited);
    return ans;
}

void solve(i32 n,i32 m,i32 p){
    rep(i,0,n) rep(j,0,m) see(grid[i][j]);
    vector<vector<i32>> visited(n,vector<i32>(m,-1));
    vector<i32> ans = dfs(0,p-1,n,m,1,visited);
    // debug(ans);
    if(ans[0]==1){
        putln(ans[1],"step(s) to exit");
    }else{
        putln(ans[1],"step(s) before a loop of",ans[2],"step(s)");
    }
}
