...
// void dfs(i32 node,vector<i32> &visited,i32 val){
...
//articulation points
i32 root;
vector<bool> articulationPoints(MAXN,false);

void dfs(i32 node,i32 &articulPoints,i32 &rootChildren,i32 &dfsCounter,i32 par,vector<i32> &in,vector<i32> &low){
    // debug(node,articulPoints,rootChildren,dfsCounter,par,in,low,articulationPoints);
    in[node] = dfsCounter++;
    low[node] = in[node];
    // debug(node,articulPoints,rootChildren,dfsCounter,par,in,low,articulationPoints);
    for(auto &u : adj[node]){
        if(in[u]==-1){
            ...
            dfs(u,articulPoints,rootChildren,dfsCounter,node,in,low);
            // debug(node,u,articulPoints,rootChildren,dfsCounter,par,in,low,articulationPoints);

            if((node!=root) && low[u]>=in[node]) articulationPoints[node] = true;
            // debug(articulationPoints,u,node);
...
void solve(i32 n){
    // debug(n);
    articulationPoints.assign(MAXN,false);
    ...
    // debug(adj);
    i32 dfsCounter=1,articulPoints=0;
    vector<i32> in(n+1,-1),low(n+1,-1);
    rep(i,1,n+1){
        if(in[i]==-1){
            i32 rootChildren = 0;
            root = i;
            // debug(in,low,dfsCounter,articulPoints,root);
            dfs(i,articulPoints,rootChildren,dfsCounter,-1,in,low);
            // debug(rootChildren,articulationPoints);
            if(rootChildren>1) articulationPoints[i]=true;
        }
    }
    // debug(articulationPoints,in,low);
    rep(i,1,n+1){
        if(articulationPoints[i]) articulPoints++;
    }
    putln(articulPoints);
    // dfs()
