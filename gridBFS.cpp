...
void solve(){
    ...
    while(!q.empty()){
        ...
        rep(i,0,4){
            i32 stp = grid[u][v]-'0';
            i32 k = u+(stp*steps[0][i]), l = v+(stp*steps[1][i]);
            // debug(u,v,k,l,stp,steps[0][i],steps[1][i]);
            if((k<0)||(l<0)||(k>=n)||(l>=m)||(visited[k][l]!=-1)) continue;
            visited[k][l]=visited[u][v]+1;
            q.push({k,l});     
        }
    }
    // debug(visited);
    putln(visited[n-1][m-1]);
}
