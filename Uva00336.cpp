...
void solve(i32 n){
    ...
    while(1){
        ...
        // debug(mp.size());
        while(!q.empty()){
            auto [tpx,tpy] = q.front();
            q.pop();
            st.insert(tpx);
            if(tpy==0||mp.count(tpx)==0) continue;
            tpy--;
            for(auto &x:mp[tpx]){
        ...
        if(mp.count(u)==0) k++;
        // debug(k,st.size());
        putln("Case",tostring(t)+":",k-st.size(),"nodes not reachable from node",u,"with TTL =",tostring(v)+".");
        t++;
    }
