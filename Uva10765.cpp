...
void dfs(i32 node,vector<i32> &in,vector<i32> &low,vector<i32> &articulationPoints,i32 par){
    // debug(node,dfsCounter,in,low,par,articulationPoints);
    in[node]=dfsCounter++;
    low[node] = in[node];
    // debug(node,dfsCounter,in,low,par);
    for(auto &u:adj[node]){
        if(in[u]==-1){
            // debug(node,u);
            if(node==root) rootChildren++;
            dfs(u,in,low,articulationPoints,node);
            // debug(node,u,in,low);
            if((in[node]<=low[u])){
                // debug(node,in,low,u);
                if(articulationPoints[node]==-1) articulationPoints[node] = 2;
                else articulationPoints[node]++;
            }
...
bool comparator(pair<i32, i32> &a,pair<i32, i32> &b){
...
void solve(i32 n,i32 m){
    ...
    // debug(adj);
    vector<i32> articulationPoints(n,-1),in(n,-1),low(n,-1);

    dfs(0,in,low,articulationPoints,-1);
    // debug(rootChildren,articulationPoints);
    articulationPoints[0]=rootChildren;
    rep(i,0,n){
        if(articulationPoints[i]==-1) articulationPoints[i] = 1;
    }
    // debug(articulationPoints,in,low);
    vector<pair<i32, i32>> arr(n);
    rep(i,0,n){
        arr[i] = {articulationPoints[i],i};
    }
    sort(all(arr),comparator);
    rep(i,0,m){
        putln(arr[i].second,arr[i].first);
    }
    cout<<"\n";
}
